    ...
    <ShadowCard sx={{ width: "100%", padding: "15px 10px", fontSize: "16px" }} >
      <Stack>
        ...
        <Grid container rowSpacing={1} columnSpacing={4} >
          <Grid item xs={6}>
            <span>程序：</span><span>{rule.process}</span>
          </Grid>
          <Grid item xs={6}>
            <span>操作：</span><span>{rule.action}</span>
          </Grid>
        </Grid>
      </Stack>
      <BaseAlertDialog open={deleteOpen} title='警告' message='是否删除此规则？' onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} />
